...
def movie_handler(cli_args):
    ...
    output_movie_folder_name = "{} ({})".format(movie_name, movie_year)
    output_movie_folder_path = os.path.join(cli_args.plex_location, 'movies', output_movie_folder_name)

    # copy movie to plex
    copy_torrent_to_plex(cli_args.torrent_name, cli_args.torrent_kind, cli_args.download_location, output_movie_folder_path)
...
def show_handler(cli_args):
    ...
    copy_torrent_to_plex(cli_args.torrent_name, cli_args.torrent_kind, cli_args.download_location, output_show_folder_path)
...
def copy_torrent_to_plex(torrent_name, torrent_kind, download_location, output_folder_path):
    ...
    if torrent_kind == 'multi':
        ...
        copyfile(os.path.join(download_location, torrent_name),
                 os.path.join(output_folder_path, torrent_name))
    else:
        raise ValueError("torrent_kind should either be 'single' or 'multi' and not {}".format(torrent_kind))
