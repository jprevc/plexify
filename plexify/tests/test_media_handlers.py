...
def make_cli_args_fixture(
    ...
    def make_cli_args(
        ...
        use_download_folder_as_label=False,
    ) -> MockCliArgs:
        if not subtitles:
            subtitles = []

        if use_download_folder_as_label:
            # simulate case, when user selects use_download_folder_as_label argument.
        ...
        return MockCliArgs(
            ...
            label=label,
            use_download_folder_as_label=use_download_folder_as_label,
...
@pytest.mark.parametrize('use_download_folder_as_label', [True, False])
